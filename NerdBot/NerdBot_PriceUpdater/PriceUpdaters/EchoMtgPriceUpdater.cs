...
namespace NerdBot_PriceUpdater.PriceUpdaters
{
    public class EchoMtgPriceUpdater : IPriceUpdater
    {
        ...
        public void UpdatePrices(object sender, DoWorkEventArgs e)
        {
            BackgroundWorker worker = sender as BackgroundWorker;

            // Get sets from IMtgStore
            this.mLoggingService.Debug("Getting sets..."); //TODO Logging configuration fails
            List<Set> sets = this.mMtgStore.GetSets().Result;
            this.mLoggingService.Debug("Got {0} sets!", sets.Count);

            foreach (Set set in sets)
            {
                string url = string.Format(cUrl, set.Code);

                Console.WriteLine("Getting page for set '{0}' from '{1}'...",
                    set.Code,
                    url);
